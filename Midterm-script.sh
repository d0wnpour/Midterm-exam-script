...
function dependency_checker() {
    if [ $1 -ne 4 ]; then
        echo "Usage: $2 CODE_REPO_URL CODE_BRANCH_NAME REPORT_REPO_URL REPORT_BRANCH_NAME"
        exit 1
    fi
...
while true; do
    ...
    git pull
    git switch $BRANCH_CODE_REPOSITORY
...
"
                ...
                cd $CODE_REPOSITORY_PATH
                TAG=$BRANCH_CODE_REPOSITORY-ci-success
